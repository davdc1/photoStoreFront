                    ...
                    {this.state.products.map((product, index) => {
                        if(index <= 4){
                            if(index === 2){
                                ...
                                return <Link to={{pathname:`/prodpage/${product.id}`}}><div key={index} className="border border-1 rounded shadow-2xl h-72 w-72 bg-center bg-cover mx-8" style={{backgroundImage: `url(${product.imageStr})`}}></div></Link>
                            }
                            return <Link to={{pathname:`/prodpage/${product.id}`}}><div key={index} className="border border-1 rounded filter grayscale h-52 w-52 bg-center bg-cover mx-2" style={{backgroundImage:  `url(${product.imageStr})`}}></div></Link>
                        }
                    })}
